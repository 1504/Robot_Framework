...
public class VisionThreadSingleFrame {
	...
	public VisionThreadSingleFrame(VisionRunner<?> visionRunner) {
		// Spawn a thread that will process a single frame using the VisionRunner when
		// notified.
	...
	/**
	 * Creates a new vision thread that runs the given vision pipeline on request.
	 * This is equivalent to
	 * {@code new VisionThreadSingleFrame(new VisionRunner<>(videoSource, pipeline, listener))}.
	 *
	 * @param videoSource the source for images the pipeline should process
	 * @param pipeline    the pipeline to run
	 * @param listener    the listener to copy outputs from the pipeline after it
	 *                    runs
	 * @param <P>         the type of the pipeline
	 */
	public <P extends VisionPipeline> VisionThreadSingleFrame(VideoSource videoSource, P pipeline,

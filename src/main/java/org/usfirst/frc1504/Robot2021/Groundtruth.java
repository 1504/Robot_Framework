...
public class Groundtruth implements Updatable {
	 ...
	 * 
	 * @return double[] array containing: {Forward position, Right position,
	...
	/**
	 * Sets the robot's current position index. Useful for setting at the start of
	 * the match or resetting at a known point on the field.
	 * 
	 ...
	 * 
	 * @return double[] array containing: {Forward speed, Right speed, Anticlockwise
	 *         speed}
	 */
	 ...
	 * 
	 * @return double[] array containing: {Forward acceleration, Right acceleration,
	 *         Anticlockwise acceleration}
	 */
	 ...
	 * 
	 * @param data - Data format: LEFT_X, LEFT_Y, LEFT_SQUAL, RIGHT_X, RIGHT_Y,
	 *             RIGHT_SQUAL
	 */
	public void getData() {
		...
		for (int i = 0; i < motion.length; i++)
			_acceleration_samples[_sample_index][i] = (_speed_samples[_sample_index][i]
					- _speed_samples[(_sample_index + Map.GROUNDTRUTH_SPEED_AVERAGING_SAMPLES)
							% Map.GROUNDTRUTH_SPEED_AVERAGING_SAMPLES][i])
					/ elapsed_time;
	...
	public void semaphore_update() {
		...
		/*
		 * if(_raw_data != null) _logger.log(Map.LOGGED_CLASSES.GROUNDTRUTH, _raw_data);
		 * _raw_data = null;
		 */
	}
}

...
public class DriveGlide {
	...
	public double[] gain_adjust(double[] input) {
		...
		for (int i = 0; i < _last_output.length; i++) {
			boolean toward_zero = Math.abs(_last_output[i]) > Math.abs(input[i]); // Are we moving toward zero or away?
																					// (We have different gains for
			...
			double maximum_distance = looptime * magnitude * Map.DRIVE_GLIDE_GAIN[toward_zero ? 1 : 0][i]; // Maximum
																											// distance
																											// the input
																											// should
																											// move

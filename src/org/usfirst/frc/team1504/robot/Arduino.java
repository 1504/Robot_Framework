 ...
 */
	...
	*/
	public double[] arm_gradient() 
	{
		double[] RGB = {0, 0, 0};
		int returnval;
		if((Pickup.ARM_DOWN_ANGLE < _pickup.arm_get_position()) && (_pickup.arm_get_position() < Pickup.ARM_MID_ANGLE))
		{
			returnval = (int) ((_pickup.arm_get_position() / Pickup.ARM_UP_ANGLE) * 255);
			RGB[0] = 0;
			RGB[1] = returnval;
			RGB[2] = 0;
		}
		else if((Pickup.ARM_MID_ANGLE < _pickup.arm_get_position()) && (_pickup.arm_get_position() < (2 * Pickup.ARM_DOWN_ANGLE))) 
		{
			returnval = (int) ((_pickup.arm_get_position() / (2 * Pickup.ARM_DOWN_ANGLE)) * 255);
			RGB[0] = returnval;
			RGB[1] = returnval;
			RGB[2] = 0;
		}
		else if(((2 * Pickup.ARM_DOWN_ANGLE) < _pickup.arm_get_position()) && (_pickup.arm_get_position() < (Pickup.ARM_UP_ANGLE))) 
		{
			returnval = (int) ((_pickup.arm_get_position() / (Pickup.ARM_UP_ANGLE)) * 255);
			RGB[0] = 0;
			RGB[1] = 0;
